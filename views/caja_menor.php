...
<div style="display: flex;">
	...
	<img src="img/cash2.png"  width="30px"/>$<label id='total_caja_menor'></label></b>
	</div>
				...
				<form  id="registro" name="registro">	
					...
					<div class="row"> 
						...
						<div class="col">
							<div class="input-group mb-3">
								...
								<option value="bank">■ Consignación</option> 

								</select>

								<input type='hidden' name="origen" value="Manual"/>
								<input type='hidden' name="codigo_estado" value="4"/>
							</div>
						</div>
						...
						<div class="col">
							<div class="input-group mb-3">
								...
								<select name="codigo_concepto" id="codigo_concepto" class="form-control"  id="codigo_sede" lang="1">
								</select>
							</div>
						</div>
					</div>
					...
					<div class="col">
							<div class="input-group mb-1">
								<div class="input-group-prepend">
									<span class="input-group-text" >Orden  de </span>
								</div>
								<select name="motivo" class="form-control"  id="motivo" lang="1">
						...
						<div class="col">
							<div class="input-group mb-3">
								...
								<input name="numero" type="text"  class="form-control" placeholder="" lang="1">	
							</div>
						</div>
					...
					<div class="row">
						...
						<div class="col-8">
							<div class="input-group mb-3">
								<div class="input-group-prepend">
									<span class="input-group-text" >Servicio / Adquisión</span>
								</div>
								<textarea
									name="observacion"
									rows="3"
...
<script type="text/javascript">
	...
	if(codigo_rol==1){
		...
		filtro_index= ' where cm.codigo_sede='+ codigo_sede +' order by cm.fecha desc';
	}
	...
	var	form=[
		...
		{	id:1,
			...
			modal:'#modal_reg',
			div_listado:'div_listado',	
			tabla:'tbl_caja_menor',
			codigo:'codigo_transaccion',
			filtro:' where cm.codigo_sede='+ codigo_sede +' order by cm.fecha desc',
			referencia:'',
			listado:'listado_caja_menor',
			button:'#btn_registro',
			cierre_modal:1
	...
	$(document).ready(function() {
		...
		if (parseInt(codigo_rol)==1){
			...
			$("#codigo_sede_filtro").bind("change",function(){
				filtro=" where cm.codigo_sede="+ $("#codigo_sede_filtro").val() +" order by cm.fecha desc";
	...
	function load_modal(n,s,r){
		...
		case 1:
			...
			rellenar_select("tbl_sedes","codigo_sede","nombre",filtro_sede,"codigo_sede", filtrar_sede);
			...
			$("#sct_tipo").on("change",function(){
				rellenar_select("tbl_caja_menor_conceptos","codigo_concepto","descripcion","tipo='"+ $("#sct_tipo").val()+"'","codigo_concepto");

					if($("#sct_tipo").val()=="bank"){
						rellenar_select("tbl_proveedores","codigo_proveedor","razon_social", " banco=1","codigo_proveedor");
					}else{

						if($("#sct_tipo").val()=="in"){
							rellenar_select("tbl_proveedores","codigo_proveedor","razon_social", " codigo_proveedor=1","codigo_proveedor");
