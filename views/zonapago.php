...
if (isset($_FILES['archivo']['name'])){
     ...
     if($_FILES['archivo']['name'] == null){
        echo "Debe cargar una foto del comprobante de pago obligatoriamente para en contrato nro: ".$_POST["codigo_contrato"];
        exit();
    }
...
if (is_dir($upload_dir) && is_writable($upload_dir)) {
    ...
    // sql de guardado
        ...
        echo "Debe cargar una foto del comprobante de pago obligatoriamente.";
        exit();
    }
        ...
        if(isset($_GET['n'])){
           ...
           $sql='SELECT 
            ...
            sp.codigo_contrato=c.codigo_contrato
             ...
             WHERE cl.tipo_documento="'. $tipo_documento .'" 
            AND cl.identificacion="'.$numero. '" AND c.activo=1';
...
echo $sql;
            ...
            $dataComplete=$result->fetch_all(MYSQLI_ASSOC); 


            foreach($dataComplete as $kdata => $vdata){


                if($vdata['saldo'] == 0 || $vdata['saldo'] == ''){
                    $saldoPositivo=0;
                }else{
                    $saldoPositivo=$row[7];
                }
                ...
                $fecha_fin_history=$vdata['Fecha Fin'];
                $cod_contrato=$vdata['codigo_contrato'];
                $valor_pago_semanal=$vdata['Valor'];
    
                $pagosCiclo=0;
    
                $sql="SELECT pago_realizado as pago FROM tbl_contrato_pagos 
                WHERE fecha_fin_history="."'".$fecha_fin_history."'"." 
                AND codigo_contrato=".$cod_contrato;
                ...
                echo '<form id="enviar-archivo" action="https://mototrabajo.com/zona-de-pagos/" class="elementor-form" method="post" enctype="multipart/form-data">
                    ...
                    <div>
                     ...
                     <b>Ciclo :</b> '.  $vdata['Fecha Inicio'] .'<b> - </b>'.$vdata['Fecha Fin'].'<br>
                    <b>Moto :</b> '.  $vdata['Moto'] .'<br>
                    <input name="valor" type="hidden" value="'.$vdata['Valor'].'"/>
                    <input name="codigo_contrato" type="hidden" value="'.$vdata['codigo_contrato'].'"/>
                    </div>
                ...
                <div class="elementor-form-fields-wrapper elementor-labels-above">
                   <div class="elementor-field-type-select elementor-field-group elementor-column elementor-field-group-name elementor-col-50 elementor-field-required">
                        ...
                        <option value="Seleccione tipo de documento">Seleccione tipo de documento</option><option value="CC">Cedula de ciudadanía</option><option value="EX">Cedula de Extranjería</option><option value="PA">Pasaporte</option></select>
                            ...
                            <input type="text" name="n" id="form-field-field_65ac13a" class="elementor-field elementor-size-lg  elementor-field-textual" placeholder="Numero de documento" required="required" aria-required="true" min="" max=""></div>
...
echo "Este es el error ". $_FILES["archivo"]["error"];
 ...
 echo '<pre>';
            print_r( $data=$result->fetch_all(MYSQLI_ASSOC)); 
            echo '</pre>';
