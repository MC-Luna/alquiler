<?php
	...
	echo listado_consulta("listado_contratos3"," 1",1);
		...
		if (empty($res)) {
			...
			if ($res_consulta->num_rows>0) {
				...
				if ($res_opcion->num_rows>0) {
					...
					echo "<pre>";
					print_r($datos_consulta_opciones);
					echo "</pre>";
				...
				$tabla="<table border='1' class='table table-striped ' style='white-space: nowrap;'>";
							...
							$fila.=" >";
								...
								foreach ($value as $valor) {
									...
									if ($i < $num_campos_ocultos) {
										...
										$td_opcion=str_replace("<<".$i.">>",$value[$i],$td_opcion);
										...
										//foreach ($datos_columnas[$x] as $key => $val) {
											...
											//if ($o < $num_campos_ocultos) {
												...
												//$td_opcion=$td_opcion.$val;
											//	$td_opcion.=$key . "=" .$val. " jog <br>"; 
											//}
											//$o++;	
									//	}
