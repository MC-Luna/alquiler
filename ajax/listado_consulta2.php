...
//		error_log($consulta, 1,"jorgemario.com@gmail.com");

		$res_consulta=$conexion->ejecutar_sql($consulta);
		$res_consulta_forArray=$conexion->ejecutar_sql($consulta);

		
		$arrListados=array('listado_pagos', 'listado_contratos2','listado_leads_nuevo');
					...
					foreach ($columnas as $key => $value) {
						...
						if($i>=$res[0]["campos_ocultos"]){
							...
							if(in_array($key, $columnasOmitir) != true ){
								if($key=="color" && $_POST['consulta'] == "listado_contratos2"){
								}else{
									$tabla.="<td>" . $key . "</td>";
								}
							}
					...
					foreach ($datos_consulta as $value) {
						...
						$tabla.="<tr class='item' ";
							...
							foreach ($value as $valor) {
								...
								}else if($_POST['consulta'] == "listado_contratos2"){
