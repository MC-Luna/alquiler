...
// ------------ FIN FUNCIONES
    ...
    where p.codigo_cobro is null and p.codigo_contrato=".$_POST["codigo_contrato"]." order by p.fecha_registro desc";
    ...
    if(!$result=$db->query($sql)){ 
        ...
        if(!$result=$db->query($sql)){ 
            ...
            $sql='SELECT * FROM tbl_contrato_pagos WHERE codigo_cobro is null AND codigo_contrato = '.$_POST["codigo_contrato"]." AND fecha_fin_history= '".$fechaFin."'";
        ...
        set validado=1, numero_soporte="'.$_POST["soporte_pago"].'", observaciones="'.$observacion.'",
        forma_pago="'.$_POST["forma_pago"].'", pago_realizado="'.$PagoRealizado.'", 
        fecha_inicio_history="'.$fechaInicio.'", saldo_positivo="'.$positivoNuevo.'", 
        fecha_fin_history="'.$fechaFin.'", codigo_banco='.$_POST["codigo_banco"].' , codigo_cuenta='.$_POST["codigo_cuenta"].' , codigo_usuario='.$_POST["codigo_usuario"].'
